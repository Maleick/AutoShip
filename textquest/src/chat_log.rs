...
impl ChatLogWriter {
    ...
    fn open_writer(&self, path: &Path) -> BufWriter<File> {
        let file = OpenOptions::new()
            ...
            .open(path)
            .unwrap_or_else(|_| {
                let temp = std::env::temp_dir().join("textquest").join("fallback.log");
                File::create(&temp).expect("fallback log must be creatable");
                File::open(&temp).expect("fallback log must be openable")
            });
        BufWriter::new(file)
    }

...
pub mod profile {
    ...
    /// Number of spellbook slots in `BaseProfile::SpellBook`.
    pub const SPELL_BOOK_SLOTS: usize = 1280;
    /// Alias for `SPELL_BOOK_SLOTS` — total slots between `SpellBook` and `MemorizedSpells`.
    pub const SPELL_BOOK_SLOT_COUNT: usize = SPELL_BOOK_SLOTS;
    /// Visible memorized spell gem slots in `BaseProfile::MemorizedSpells`.
    pub const MEMORIZED_SPELL_GEMS: usize = 15;
    /// Alias for `MEMORIZED_SPELL_GEMS` — visible spell-gem slots used by the live client UI.
    pub const MEMORIZED_SPELL_GEM_COUNT: usize = MEMORIZED_SPELL_GEMS;

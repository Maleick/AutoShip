...
pub mod profile {
    ...
    /// Total spell-book slots stored in `BaseProfile::SpellBook`.
    pub const SPELL_BOOK_SLOTS: usize = 1280;
    /// Visible spell-gem slots used by the live client UI (alias: `MEMORIZED_SPELL_GEMS`).
    pub const MEMORIZED_SPELL_GEM_COUNT: usize = 15;
    /// Visible memorized spell gems stored in `BaseProfile::MemorizedSpells` (alias: `MEMORIZED_SPELL_GEM_COUNT`).

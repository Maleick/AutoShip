...
impl DanNetPeer {
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        if self.name.trim().is_empty() {
            bail!("DanNet peer name must not be empty");
        }
        if self.name.len() > MAX_PEER_NAME_LEN {
            bail!("DanNet peer name exceeds {MAX_PEER_NAME_LEN} chars");
        }
        if self.host.trim().is_empty() {
            bail!("DanNet peer host must not be empty");
        }
        if self.port == 0 {
            bail!("DanNet peer port must be non-zero");
        }
        Ok(())
    }
}
    ...
    match verb.as_str() {
        ...
        "/dgae" => {
            if rest.is_empty() {
                ...
                Some(Ok(DanNetCommand::AllExecute {
...
fn parse_group_global_all_execute(rest: &str) -> Result<DanNetCommand, String> {
    ...
    Ok(DanNetCommand::GroupGlobalAllExecute {
        group: group.to_string(),
        command: command.to_string(),
    })
}
...
mod tests {
    ...
    fn parse_dnet_info() {
        assert_eq!(
            parse_dannet_command("/dnet info"),
            Some(Ok(DanNetCommand::Info))
        );
    }
    ...
    fn parse_dn_peer_command() {
        assert_eq!(
            ...
            Some(Ok(DanNetCommand::Peer {
                peer: "Warrior".to_string(),
    ...
    fn parse_dg_group_command() {
        assert_eq!(
            ...
            Some(Ok(DanNetCommand::Group {
                group: "healers".to_string(),
    ...
    fn parse_dggaexecute_missing_command_err() {
        assert!(matches!(
            parse_dannet_command("/dggaexecute maintank"),
            Some(Err(_))
        ));
    }
    ...
    fn to_wire_all_execute() {
        let cmd = DanNetCommand::AllExecute {
            command: "/sit".to_string(),
        };
        assert_eq!(
            cmd.to_wire_message("Cleric01"),
    ...
    fn peer_validate_rejects_oversized_name() {
        let p = DanNetPeer {
            name: "x".repeat(MAX_PEER_NAME_LEN + 1),
            host: "10.0.0.1".to_string(),
            port: 2114,
        };
        assert!(p.validate().is_err());

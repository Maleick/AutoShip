#!/usr/bin/env bash
# AutoShip Hermes PR creation — create PR from completed delegate_task workspace
# Dependency graph: lib/common.sh (optional), pr-title.sh, update-state.sh
# Leaf callers: runner.sh (after delegate_task completes), manual invocation
set -euo pipefail
...
RESULT_PATH="${3:-$WORKTREE_PATH/HERMES_RESULT.md}"
MODE="${AUTOSHIP_PR_MODE:-dry-run}"

REPO_ROOT=$(autoship_repo_root) || exit 1

# ── Auto-sync: pull latest plugin code before PR creation ──
...
if [[ ! -s "$RESULT_PATH" ]]; then
  echo "VERDICT: FAIL - HERMES_RESULT.md missing"
  exit 1
fi
if [[ -L "$RESULT_PATH" ]]; then
  echo "VERDICT: FAIL - HERMES_RESULT.md must not be a symlink"
  exit 1
fi

REAL_WORKTREE=$(canonical_dir "$WORKTREE_PATH")
REAL_RESULT=$(canonical_file "$RESULT_PATH") || {
  echo "VERDICT: FAIL - cannot resolve HERMES_RESULT.md"
  exit 1
}
case "$REAL_RESULT" in
  "$REAL_WORKTREE"/*) ;;
...
# Determine target repo from state file or derive from origin remote
...
if [[ -z "$REPO_SLUG" ]]; then
  echo "Error: cannot determine target repo slug" >&2
  exit 1
fi
...
if [[ -x "$SCRIPT_DIR/pr-body.sh" ]]; then
...
else
  {
    ...
    cat "$RESULT_PATH"
    printf '\n\nCloses #%s\n\n' "$ISSUE_NUMBER"
    printf 'Dispatched by AutoShip (Hermes).\n'
  } >"$BODY_FILE"
fi
...
(
  ...
  if [[ -n "$CHANGED_PATHS" ]]; then
    ...
    if ! git diff --cached --quiet; then
      git commit -m "$TITLE" -m "Closes #$ISSUE_NUMBER" -m "Dispatched by AutoShip (Hermes)."
    fi
  fi

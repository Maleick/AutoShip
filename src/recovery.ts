 ...
 */
export const DEFAULT_RECOVERY_MAP: Record<ErrorKind, RecoverySuggestion> = {
  network: {
 ...
 */
export function classifyError(message: string, code?: string): ErrorKind {
  const msg = message.toLowerCase();
  const errCode = (code ?? "").toLowerCase();

  if (
    errCode.startsWith("econn") ||
    errCode === "enetunreach" ||
    errCode === "eai_again" ||
...
export function getRecoverySuggestion(
  kind: ErrorKind,
  map: Record<ErrorKind, RecoverySuggestion> = DEFAULT_RECOVERY_MAP,
): RecoverySuggestion {

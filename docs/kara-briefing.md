...
| ------------- | -------- | -------------- | ------------------------------------------ |
| `dmft`        | `bin`    | `dmft.exe`     | TUI dashboard + orchestrator + IPC server  |
| `dmft-dll`    | `cdylib` | `dmft_dll.dll` | Injected payload — game hooks, combat, nav |
| `dmft-common` | `lib`    | (linked)       | Shared types, offsets, IPC protocol        |
...
cargo build --release        # Release build → target/release/dmft.exe + dmft_dll.dll
...
2. **Build release binaries** — confirm `cargo build --release` produces clean dmft.exe + dmft_dll.dll
...
| File                              | Why it matters                                                                        |
| --------------------------------- | ------------------------------------------------------------------------------------- |
| `dmft-common/src/offsets.rs`      | All EQ memory addresses. **Never use raw values** — always `rebase()` to runtime base |
| `dmft-dll/src/hooks/game_loop.rs` | The heartbeat — per-frame command dispatch in the game loop                           |
| `dmft-dll/src/hooks/render.rs`    | Render mode control hook on `CDisplay::RealRender_World`                              |
| `dmft-dll/src/hooks/dx11_null.rs` | DX11 null device for GPU memory reduction                                             |
| `dmft-dll/src/ipc/pipe.rs`        | Named pipe server (DLL side)                                                          |
| `dmft/src/ipc/`                   | Named pipe client (orchestrator side)                                                 |
| `dmft/src/inject/`                | DLL injection staging                                                                 |
| `dmft-dll/src/combat/`            | All 17 class combat strategies                                                        |
| `dmft-dll/src/nav/`               | Navigator FSM                                                                         |
| `dmft-dll/src/login/`             | Login state machine                                                                   |
...
- **Logging**: Use `tracing` crate, never `log`. `println!` only in user-facing CLI output (`dmft/src/main.rs`).
...
- **Repo path**: `C:\Users\xmale\Projects\DMFT`
- **Git submodules**: `third_party/eqlib` and `third_party/macroquest` — run `git submodule update --init --recursive` after checkout if doing offset/struct work
- **Sync script**: `scripts/sync-frostreaver.ps1` — syncs Claude Code settings and rebuilds DMFT

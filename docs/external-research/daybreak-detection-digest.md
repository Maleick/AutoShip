...
This digest feeds `M5` Anti-Cheat. It separates official Daybreak signals from secondary community reporting.
...
- official Daybreak policy can create or tighten `M5` gates immediately
...
- separate module, hook, packet, timing, and environment risks
- require new `M5` tasks to name which exposure categories they touch and whether the confidence is high, medium, or low
...
- keep broader relay or session-routing work on the lowest-exposure control path
- block operator-facing orchestration claims that silently widen packet, hook, or movement risk
...
## Required Labels For New `M5` Tasks

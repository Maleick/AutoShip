...
6. Issue #2462 audit result: openvanilla `16605f7c43ed3284b791547c2df3bff0f9aa7310` fixes foreground handoff by attaching the current thread to the current foreground thread (`AttachThreadInput`) around `BringWindowToTop`/`SetForegroundWindow`, then validating with `GetForegroundWindow()`. TextQuest has no `SetForegroundWindow`, `AttachThreadInput`, or `AllowSetForegroundWindow` paths in `textquest/src/window_title_runtime.rs`, `textquest/src/launcher`, or `textquest-dll/src/overlay`, so no matching fix was required.

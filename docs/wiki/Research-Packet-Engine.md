...
Curated `M5` send/receive pipeline inventory for the TextQuest packet engine capability boundary.

This document turns the April 2026 network architecture import into an explicit send/receive surface map for TextQuest. It does not treat imported reverse-engineering notes as proof that TextQuest already supports packet-first execution. The current evidence state for all receive-side and packet-sender rows remains `Needs Live Proof` unless explicitly marked otherwise.

## Scope

- map the EQ outbound (send) and inbound (receive) packet layers to TextQuest's current control posture
- label each layer as `In-process`, `Packet candidate`, or `Blocked` relative to existing TextQuest code
...
TextQuest status: `Blocked` — TextQuest does not use this path. The navigation system writes heading and speed fields directly via in-process memory writes rather than injecting movement packets.
...
TextQuest status: `Blocked` — TextQuest currently has no counter tracking, opcode scrambler call, or packet injector. These are hard requirements for any packet-first capability work.
...
The receive pipeline is currently observational only in TextQuest. The DLL uses `ReadProcessMemory` and shared-memory IPC to observe game state rather than tapping the inbound packet stream.
...
TextQuest status: `Blocked` — TextQuest does not hook this function. Game state is observed via shared memory and EQ struct reads, not packet interception.
...
TextQuest status: `Blocked` — TextQuest does not hook these handlers. Hooking them would be the natural entry point for inbound packet observation, but no M5 task currently requires it.
...
- receive pipeline is inventoried and current TextQuest posture is documented

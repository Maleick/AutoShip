...
| --- | --- | --- | --- |
| TextQuest has a loader-safe Velketor camp baseline for the Frenzy hall. | Research-backed | `config/camps/velketors_labyrinth_frenzy.toml`, `docs/wiki/Velketors-Labyrinth-Frenzy-Camp.md` | The repo now captures safe-hall med coordinates, pull defaults, named filters, and a documented waypoint lattice. |
| TextQuest already documents the key geometry hazards that make Velketor risky for unattended routing. | Research-backed | `docs/wiki/Velketors-Labyrinth-Frenzy-Camp.md`, `docs/wiki/P99-Zone-Guide.md` | Slippery ramps, pit drops, see-invis mobs, and Lord Bob or Bledrek no-pull branches are now explicitly documented. |
...
| Velketor's Labyrinth uses a fast enough spawn cadence to keep the Frenzy route busy for a full 6-box group. | Research-backed expectation | `docs/wiki/Velketors-Labyrinth-Frenzy-Camp.md`, `docs/wiki/Frostreaver-Farming-Guide.md` | The repo records the `32:50` zone timer guidance and named cluster, but still lacks live placeholder and wait-time samples. |
| The documented Frenzy route is safe for unattended 6-box farming. | Needs live proof | Issue `#1720`, `docs/wiki/Velketors-Labyrinth-Frenzy-Camp.md` | This is the remaining blocker: the route, leash behavior, med thresholds, and return path all still need a live session. |
...
- `config/camps/velketors_labyrinth_frenzy.toml` defines the current runtime
  ...
  ignore lists.
- `docs/wiki/Velketors-Labyrinth-Frenzy-Camp.md` records the 28-point route

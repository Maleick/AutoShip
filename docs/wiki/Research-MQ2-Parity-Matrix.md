...
| `/nav door [name\|id] [click]`      | Navigate to door, optionally interact | **Partial** — `InteractDoor` IPC cmd opens nearest door via `/doortarget` + `/click left door`; TUI `:door` / `:click door` | Navigation to door position pending (M7 DoorsManager) | P2 | Medium |
| `/nav item [click]`                 | Navigate to ground item               | **Partial** — `ClickObject` IPC cmd clicks nearest item via `/click left item`; TUI `:click`               | Navigation to item position pending (M7 ground spawns) | P2 | Low |

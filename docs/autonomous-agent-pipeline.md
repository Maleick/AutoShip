...
- Operator: creates issues, curates roadmap milestones, and adds routing or risk labels.
...
- Nightly research automation: separate from issue execution. It updates docs and roadmap materials first, then promotes mature checkpoint work into milestones, issues, and PR follow-ups.

## Queue Contract

GitHub issues, linked PRs, and milestones are the active tracking surfaces. The repo remains authoritative, especially [`docs/implementation-roadmap.md`](implementation-roadmap.md). GitHub Projects are historical/retiring and must not be treated as live queue state.

An issue enters the autonomous queue only when both are true:

1. The issue has label `agent:ready`.
2. The issue body is concrete enough to execute without reopening scope or missing-verification questions.
...
3. Assign the relevant roadmap milestone when it is known.
4. Leave the issue unlabeled until it is ready for hands-off execution.
5. When ready, add `agent:ready`.
6. Optionally add:
...
7. The worker claims exactly one issue per run:
   ...
   - branch from `master`
8. After verification succeeds, the worker opens a non-draft PR into `master`, links the issue, and summarizes verification in the PR body.
9. If blocked, the worker adds `agent:blocked`, removes `agent:working`, and leaves a concrete unblock comment.
10. After merge, close the issue or move it to the next milestone checkpoint, and clear any stale working or blocked labels.
...
- If an issue gets stuck with `agent:working`, either let the current worker finish or swap it to `agent:blocked` before re-queueing it.

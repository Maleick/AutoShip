...
- Level-too-low zones restore player to previous safe coordinates
  ...
  ⎿  Response:
       ...
       Function Address: 0x140176460

       Original Name: FUN_140176460

       New Name: CEverQuest__HandleDynamicZoneSelect

       Confidence Level: 99%+

       Analysis Summary

       This function handles dynamic zone selection via command/chat interface in EverQuest. It processes player requests to select a specific instance of a dynamic zone.

       Key Evidence

       1. Network Packet Opcode 0x4876: A distinct packet opcode specifically for zone selection (different from 0x3937 used for full zone requests or 0x1ca1 used by the UI window)
       2. Zone Information Processing:
         - Calls WorldData__GetZoneName() to retrieve the zone name from zone ID
         - Calls FormatZoneMessage() to format zone information for display
         - Displays zone info via CEverQuest__dsp_chat() with message type 0x111
       3. "fewestplayers" Option:
         - Compares input parameter with string "fewestplayers" at 0x1408adf90
         - Sets a flag at _pinstLocalPC + 0x2ede based on this comparison
         - This allows automatic selection of the least populated instance
       4. Packet Structure:
         - Allocates 8 bytes but only uses 2 bytes for the packet
         - Sends a simple zone selection ID to the server
         - Uses global buffer g_ZoneRequestPacketBuffer at 0x140e770f0
       5. Function Relationships:
         - Related to CDynamicZoneWnd__HandleZoneSelection (UI-based selection)
         - Distinct from CEverQuest__SendZoneRequest (full zone transfer)
         - Part of the dynamic zone system alongside CDynamicZoneWnd__* functions
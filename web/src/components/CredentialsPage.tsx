  ...
  if (status === "locked") {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] uppercase tracking-widest border border-yellow-500/40 bg-yellow-900/20 text-yellow-400 font-rune">
        <LockSimple size={10} weight="fill" /> Offline
      </span>
    );
  }
  return (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-void border border-red-900/50 p-6 w-full max-w-sm shadow-[0_0_30px_rgba(239,68,68,0.15)]">
        <h3 className="font-archaic text-lg text-red-400 mb-2">Remove Credential?</h3>
        <p className="text-white/70 text-sm mb-6">
          Permanently remove the credential for{" "}
...
function TestConnectionButton({ name: _name }: { name: string }) {
  ...
  const handleTest = async () => {
    setTesting(true);
    try {
      // Simulated test connection — in production, this would call
      // a backend API to validate the credential
  ...
  if (result === "success") {
    return (
      <button
        disabled
        className="px-2 py-1 text-[10px] font-rune text-green-400 border border-green-500/40 bg-green-900/20"
      >
        ✓ Connected
      </button>
    );
  ...
  if (result === "error") {
    return (
      <button
        disabled
        className="px-2 py-1 text-[10px] font-rune text-red-400 border border-red-500/40 bg-red-900/20"
      >
        ✗ Failed
      </button>
    );
  ...
  return (
    <button
      onClick={handleTest}
      disabled={testing}
      className="px-2 py-1 text-[10px] font-rune text-white/60 border border-white/20 hover:border-white/40 hover:text-white transition-colors disabled:opacity-50 flex items-center gap-1"
    >
      <PlugsConnected size={12} /> {testing ? "Testing…" : "Test"}
    </button>
  );
}

// ─── Main CredentialsPage ──────────────────────────────────────────────────────
...
export default function CredentialsPage() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "server" | "status">("name");
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast(`Credential for '${deleteTarget}' removed`, "ok");
    } catch (e) {
  ...
  const handleImport = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast(`Imported ${count} credential(s)`, "ok");
    } catch (e) {
  ...
  return (
    <section className="flex-1 h-full flex flex-col relative z-20 min-w-[700px]">
      ...
      <header className="h-16 border-b border-white/10 flex items-center justify-between px-6 bg-violet/30 backdrop-blur-md">
        <div className="flex items-center gap-4">
          <div className="w-8 h-8 rounded border border-magentadark flex items-center justify-center bg-void">
            <LockSimple weight="fill" className="text-magentaglow" />
          </div>
          <div>
            ...
            <p className="text-[10px] uppercase tracking-widest text-white/50 font-rune">
              {accounts.length} stored · {statusCounts.active} online
            </p>
          </div>
        </div>
      ...
      {/* Status summary and sort controls */}
      <div className="flex items-center gap-6 px-6 py-3 border-b border-white/5 bg-violet/10">
        <div className="flex items-center gap-6">
          <span className="text-xs font-rune text-green-400 flex items-center gap-1.5">
            <CheckCircle size={12} weight="fill" /> {statusCounts.active} Online
          </span>
          <span className="text-xs font-rune text-yellow-400 flex items-center gap-1.5">
            <LockSimple size={12} weight="fill" /> {statusCounts.locked} Offline
          </span>
          <span className="text-xs font-rune text-red-400 flex items-center gap-1.5">
            <Warning size={12} weight="fill" /> {statusCounts.banned} Error
          </span>
        </div>
        <div className="ml-auto flex items-center gap-4">
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as "name" | "server" | "status")}
            className="bg-void border border-white/15 text-white text-xs px-2 py-1.5 focus:outline-none focus:border-magentaglow font-rune"
          >
          ...
          <input
            ...
            placeholder="Search…"
            className="bg-void border border-white/15 text-white text-xs px-3 py-1.5 w-40 focus:outline-none focus:border-magentaglow font-rune placeholder:text-white/25"
          />
        </div>
      </div>
        ...
        {!loading && !error && sorted.length === 0 && (
          <div className="flex flex-col items-center justify-center h-48 gap-3 text-white/30">
            ...
            <p className="font-rune text-sm">
              {search ? "No credentials match the filter" : "No credentials stored yet"}
            </p>
            {!search && (
              <button
                onClick={() => setModalAccount(null)}
            ...
            <tbody>
              {sorted.map((account, i) => (
                ...
                >
                  ...
                  <td className="px-3 py-3">
                    <TestConnectionButton name={account.name} />
                  </td>
                  <td className="px-5 py-3">

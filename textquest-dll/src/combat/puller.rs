...
use textquest_common::combat::PullMethod;
...
impl Puller {
    ...
    pub fn next_target(&self, ctx: &CombatContext) -> Option<u32> {
        let player_pos = textquest_common::nav::Waypoint::new(ctx.player.x, ctx.player.y, ctx.player.z);
        ctx.nearby_enemies.iter()
            .filter(|npc| !self.recently_pulled.contains(&npc.spawn_id))
            .map(|npc| {
                let dist = player_pos.distance_2d(&textquest_common::nav::Waypoint::new(npc.x, npc.y, npc.z));
                (npc.spawn_id, dist)
            })

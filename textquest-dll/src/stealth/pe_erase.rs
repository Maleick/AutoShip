...
use windows::Win32::System::Memory::{
    MEM_COMMIT, MEMORY_BASIC_INFORMATION, PAGE_PROTECTION_FLAGS, PAGE_READWRITE, VirtualProtect,
    VirtualQuery,
};
...
pub fn erase_pe_headers(dll_base: *mut u8) -> Result<(), String> {
    ...
    unsafe {
        ...
        if mbi.State != MEM_COMMIT {
            return Err("DLL base not committed".into());
        }
        if *(dll_base as *const u16) != 0x5A4D {
            return Err("Invalid DOS signature".into());
        }
        let e_lfanew = *(dll_base.add(0x3C) as *const i32);
        if e_lfanew <= 0 || e_lfanew as usize > mbi.RegionSize {
            return Err("Invalid e_lfanew".into());
        }
        let pe_offset = e_lfanew as usize;
        if *(dll_base.add(pe_offset) as *const u32) != 0x0000_4550 {
            return Err("Invalid PE sig".into());
        }
        let size_of_headers = *(dll_base.add(pe_offset + 84) as *const u32) as usize;
        if size_of_headers < 64 || size_of_headers > 8192 {

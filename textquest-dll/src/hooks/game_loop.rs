...
fn parse_nav_waypoint_command(command: &str) -> Option<Result<NavWaypointCommand, String>> {
    ...
    match subcommand.as_str() {
        "waypoint" | "wp" => {
            ...
            // /nav waypoint <name> — shorthand recall
            ...
            if let Some(nav_waypoint_cmd) = parse_nav_waypoint_command(trimmed) {
                match nav_waypoint_cmd {
                    Ok(NavWaypointCommand::Save(name)) => {
                        let result = save_nav_waypoint(&name);
                    ...
                    Ok(NavWaypointCommand::Recall(name)) => {
                        let result = recall_nav_waypoint(&name);
                        let (success, message) = match result {
                            Ok(saved) => (
                                true,
                                format!(
...
mod tests {
    ...
    fn parse_nav_waypoint_supports_save_and_recall() {
        ...
        let record = parse_nav_waypoint_command("/nav recordwaypoint pull_spot")
            .unwrap()
            .unwrap();
    ...
    #[test]
    fn parse_nav_waypoint_ignores_non_nav_commands() {
        assert!(parse_nav_waypoint_command("/target mob").is_none());
        assert!(parse_nav_waypoint_command("/nav stop").is_none());
    }
}

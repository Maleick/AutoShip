...
- New IPC commands should update both the `Command` surface and the matching `Response` handling in `dmft-common`, whether that is a dedicated response variant or `Response::CommandResult`

## Skip
- Generated files under `third_party/`
- Demo data in `dmft/src/tui/run.rs` (macOS stubs are intentionally dummy)

...
fn draw_map_view(frame: &mut Frame, area: ratatui::layout::Rect, app: &App) {
    ...
    let (center_x, center_y, scale_x, scale_y) = if let Some(map) = &app.zone_map {
        ...
        let s = ((w as f32 - 2.0) / map.bounds.width())
...
fn map_rgb_to_color(r: u8, g: u8, b: u8, t: &Theme) -> ratatui::style::Color {
    if r == 0 && g == 0 && b == 0 { t.map_lines } else { ratatui::style::Color::Rgb(r, g, b) }
}
...
fn draw_named_tracker_panel(frame: &mut Frame, area: ratatui::layout::Rect, app: &App) {
    ...
    let u_up      = u_tracked.values().filter(|t| t.status == crate::tui::app::TrackedStatus::Up).count();
        ...
        if named.is_empty() {
            frame.render_widget(
                Paragraph::new("No named spawns detected")
                    .block(blk).style(Style::default().fg(t.text_muted)),
                na,
            );
        } else {
            let header = themed_header_row(vec!["Name", "St", "Timer"], t);
            let rows: Vec<Row> = named.iter().map(|s| {
                let (status_str, color) = if s.is_alive { ("UP", t.hp_high) } else { ("down", t.text_muted) };
                let timer_str = if let Some(death_tick) = s.death_tick {
                    let elapsed_secs = app.tick_count.saturating_sub(death_tick) * app.refresh_rate_ms / 1000;
                ...
                Row::new(vec![
                    ...
                    ratatui::widgets::Cell::from(timer_str)
                        .style(Style::default().fg(if s.is_alive { t.text_highlight } else { t.text_accent })),
                ])
            }).collect();

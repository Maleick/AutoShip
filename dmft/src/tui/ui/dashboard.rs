...
fn draw_dashboard_grid(frame: &mut Frame, area: ratatui::layout::Rect, app: &App) {
    ...
    let rows: Vec<Row> = visible.iter().map(|client| {
        ...
        if let Some(player) = &client.local_player {
            ...
            Row::new(vec![
                ratatui::widgets::Cell::from(marker)
                ...
                ratatui::widgets::Cell::from(
                    if player.mana_max > 0 { format!("{:.0}%", mana_pct) } else { "-".into() },
                )
...
fn draw_session_stats(frame: &mut Frame, area: ratatui::layout::Rect, app: &App) {
    ...
    let duration_str = format!("{:02}:{:02}:{:02}", secs / 3600, (secs % 3600) / 60, secs % 60);

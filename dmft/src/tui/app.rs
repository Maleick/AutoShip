...
mod tests {
    ...
    #[test]
    fn tab_completion_handles_unicode_case_expansion_without_panicking() {
        let mut app = App::new();
        let candidates = vec![String::from("İ"), String::from("İx")];

        app.complete_with_candidates("track ", "i", &candidates);

        assert_eq!(app.cmd_state.command_buffer, "track İ");
        assert!(app.status_message.starts_with("Matches: "));
    }
}

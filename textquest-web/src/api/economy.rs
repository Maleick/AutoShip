...
pub async fn pause_economy(State(state): State<Arc<AppState>>) -> StatusCode {
...
pub async fn resume_economy(State(state): State<Arc<AppState>>) -> StatusCode {
    let mut paused = state.economy_state.is_paused.write().await;
    *paused = false;
    StatusCode::NO_CONTENT
}
...
mod tests {
    ...
    async fn pause_sets_is_paused_true() {
        let state = demo_state();
        let status = pause_economy(State(state.clone())).await;
    ...
    async fn resume_clears_is_paused() {
        let state = demo_state();
        // First pause
        pause_economy(State(state.clone())).await;
        // Then resume
        let status = resume_economy(State(state.clone())).await;
        assert_eq!(status, StatusCode::NO_CONTENT);
        let Json(economy_status) = get_status(State(state)).await;
        assert!(!economy_status.is_paused);
    }

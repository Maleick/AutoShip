...
fn write_gm_config_to_disk(config: &GmAlertConfig) -> Result<(), String> {
    ...
    table["sound_file"] = toml_edit::value(config.sound_file.clone().unwrap_or_default());
    ...
    table["discord_webhook_url"] =
        toml_edit::value(config.discord_webhook_url.clone().unwrap_or_default());

...
function KpiCard({
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span
          className={`flex items-center gap-1 font-mono text-[11px] ${
            deltaUp ? "text-state-ok" : "text-state-danger"
          }`}
        >
          {deltaUp ? (
      ...
      <div className="font-mono text-[11px] text-neriak-muted border-t border-neriak-dim/50 pt-2">
        {caption}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          <>
            <span className="text-neriak-muted">G2 Fear Core</span>
            <span className="text-neriak-dim">·</span>
            <span className="text-neriak-muted">Plane of Fear</span>
            <span className="text-neriak-dim">·</span>
            <span className="text-neriak-magenta-bright">Bertoxxulous</span>
          </>
        }
        meta={
          <>
            <span className="text-neriak-dim">uptime</span>
            <span className="text-neriak-text tabular-nums">03:47:12</span>
            <span className="text-neriak-dim">·</span>
            <span className="text-neriak-dim">tick</span>
            <span className="text-neriak-text tabular-nums">16 Hz</span>
          </>
        }
      />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          <section className="border border-neriak-dim rounded-md bg-panel overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-neriak-dim text-xs font-mono text-neriak-muted uppercase tracking-[0.15em]">
              <Activity className="w-3.5 h-3.5 text-neriak-cyan" strokeWidth={1.75} />
              active sessions
              <span className="text-neriak-dim">·</span>
              <span>{clients} clients</span>
            </div>
            <table className="w-full font-mono text-xs">
              <thead className="text-neriak-muted uppercase tracking-[0.15em] text-[9px]">
                <tr>
              ...
              <tbody>
                {rows.map((r) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-1.5 text-neriak-text">{r.character_name}</td>
                      <td className="px-3 py-1.5">
                        <span className="text-neriak-magenta">{r.class}</span>
                      </td>
                      <td className="px-3 py-1.5 text-neriak-muted">{r.zone}</td>
                      <td className="px-3 py-1.5">
          ...
          {/* Event log */}
          <section className="border border-neriak-dim rounded-md bg-panel overflow-hidden flex flex-col">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-neriak-dim text-xs font-mono text-neriak-muted uppercase tracking-[0.15em]">
              <Activity className="w-3.5 h-3.5 text-neriak-magenta" strokeWidth={1.75} />
              event log
              <span className="text-neriak-dim">·</span>
              <span className="font-mono normal-case">tail -f</span>
              <span className="ml-auto text-neriak-dim">{MOCK_LOG.length} evts</span>
            </div>
            <div className="p-2 max-h-[480px] overflow-y-auto space-y-0.5 font-mono text-[11px]">
              {MOCK_LOG.map((entry, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2 px-2 py-0.5 hover:bg-elevated/40 rounded-sm"
                >
                  <span className="text-neriak-dim tabular-nums shrink-0">{entry.ts}</span>
                  <span
                  ...
                  <span className="text-neriak-text">{entry.text}</span>
                </div>
              ))}
            </div>
            <div className="px-3 py-1.5 border-t border-neriak-dim font-mono text-[11px] text-neriak-muted flex items-center gap-2">
              <span className="text-neriak-magenta">:</span>
              <span className="text-neriak-dim">command</span>
              <span className="inline-block w-2 h-3 bg-neriak-magenta animate-pulse" />
            </div>
          </section>
        </div>
      </div>

  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          <>
            <Coins className="w-3.5 h-3.5 text-[#fbbf24]" strokeWidth={1.75} />
            <span
              className={`uppercase tracking-[0.2em] ${
                paused ? "text-[#fbbf24]" : "text-[#34d399]"
              }`}
            >
              {paused ? "paused" : "active"}
            </span>
            <span className="text-[#503c6e]">·</span>
            <span className="text-[#a096b4]">vendor loop · loot-queue · krono-sink</span>
          </>
        }
        meta={
          <>
      ...
      <div className="p-6 grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-4">
        ...
        <section className="border border-[#503c6e] rounded-md bg-[#1a0a2e]">
          <div className="flex items-center gap-2 px-3 py-2 border-b border-[#503c6e] text-xs font-mono text-[#a096b4] uppercase tracking-[0.15em]">
            <MapPin className="w-3.5 h-3.5 text-[#fbbf24]" strokeWidth={1.75} />
            vendor routes
          ...
          <table className="w-full font-mono text-sm">
            ...
            <tbody>
              {routes.map((r) => (
                <tr key={r.id} className="group border-t border-[#503c6e]/40 hover:bg-[#2d1e41]/40">
                  <td className="px-3 py-2 text-[#e2d7f4]">{r.zone}</td>
                  <td className="px-3 py-2 text-[#cc44ff]">{r.npc_name}</td>
                  <td className="px-3 py-2">
                    <div className="flex flex-wrap gap-1">
                      {r.item_categories.map((c) => (
                        <span
                          key={c}
                          className="inline-block px-1.5 py-0.5 border border-[#fbbf24]/40 bg-[#fbbf24]/5 text-[#fbbf24] rounded-sm font-mono text-[10px]"
                        >
                          {c}
                        </span>
                      ))}
                  ...
                  <td className="px-3 py-2 text-[#a096b4] text-xs italic">{r.path_notes}</td>
                  <td className="px-3 py-2">
                    <button
                      onClick={() => toggleRoute(r.id)}
                      className={`font-mono text-[11px] uppercase tracking-[0.18em] ${
                        r.enabled ? "text-[#34d399]" : "text-[#503c6e]"
                      }`}
                    >
                      {r.enabled ? "● on" : "○ off"}
                    </button>
        ...
        <aside className="space-y-4">
          <section
            className="border rounded-md bg-[#1a0a2e] p-4 space-y-3"
            style={{
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className="flex items-center gap-1 font-mono text-[11px] text-[#ef4444]">
            ...
            <div className="flex items-end justify-between">
              <div>
                <span className="font-mono text-4xl tabular-nums text-[#fbbf24]">
                  {platNow.toFixed(1)}
                </span>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[#503c6e]/50 font-mono text-xs">
              <div>
                <div className="text-[#503c6e] uppercase tracking-[0.15em] text-[10px]">
                  items distributed
                </div>
                <div className="text-[#e2d7f4] tabular-nums text-lg">
              ...
              <div>
                ...
                <div className="text-[#e2d7f4] tabular-nums text-lg">{ledger.vendor_sales}</div>
              </div>
            </div>
          </section>

          <section className="border border-[#503c6e] rounded-md bg-[#1a0a2e] p-4 space-y-3">
            <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#a096b4]">
              queues
            </div>
            {[
              ...
              <div key={label} className="space-y-1">
                <div className="flex items-center justify-between font-mono text-xs">
                  <span className="text-[#a096b4]">{label}</span>
                  <span className="tabular-nums" style={{ color }}>
                    {value}
                  </span>
                </div>
                <div className="h-1.5 bg-[#0d0618] border border-[#503c6e] rounded-sm overflow-hidden">
                  <div
                    className="h-full"
                    style={{

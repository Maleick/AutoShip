...
impl SoulCoordinator {
    ...
    /// Return a snapshot of the active personality and mapped model for a
    /// character, suitable for TUI display.
    ///
    /// Returns `None` if the character is not registered.
    #[must_use]
    pub fn character_soul_info(
        &self,
        client_id: ClientId,
    ) -> Option<CharacterSoulInfo> {
        let soul = self.souls.get(&client_id)?;
        Some(CharacterSoulInfo {
            ...
            active_model: self.config.llm.model.clone(),
            provider_kind: format!("{:?}", self.config.llm.provider),
        })
    }

    /// Hot-switch the personality traits for a registered character at runtime.
    ///
    /// Takes effect on the next tick — no restart required.
    /// Returns `true` if the character was found and updated, `false` if the
    /// `client_id` is not registered.
    pub fn set_character_personality(
        &mut self,
        client_id: ClientId,
        new_config: &CharacterSoulConfig,
    ) -> bool {
        let Some(soul) = self.souls.get_mut(&client_id) else {
            return false;
        };
        soul.traits = new_config.traits.clone();
        soul.speech_style = new_config.speech.clone();
        soul.backstory = new_config.backstory.clone();
        if let Some(edginess) = new_config.edginess {
            soul.edginess = edginess;
        }
        tracing::info!(
            character = %soul.name,
            "soul_coordinator: hot-switched personality config at runtime"
        );
        true
    }

    /// Hot-switch the LLM model used by the coordinator.
    ///
    /// Applies globally for all subsequent LLM requests. Returns the previous
    /// model name.
    pub fn set_llm_model(&mut self, model: String) -> String {
        let prev = std::mem::replace(&mut self.config.llm.model, model);
        tracing::info!(
            new_model = %self.config.llm.model,
            prev_model = %prev,
            "soul_coordinator: hot-switched LLM model"
        );
        prev
    }

    /// All registered character names, in insertion order.
    #[must_use]
    pub fn registered_character_names(&self) -> Vec<String> {
        self.souls.values().map(|s| s.name.clone()).collect()
    }

    /// Active LLM model name (as configured).
    #[must_use]
...
/// Read-only snapshot of a character's active soul/personality state for TUI
/// display.
#[derive(Debug, Clone)]
pub struct CharacterSoulInfo {
    /// Character name.
    pub name: String,
    /// Current mood state.
    pub mood: MoodState,
    ...
    /// Active personality trait vector.
    pub traits: PersonalityTraits,
    /// Edginess level for this character.
    pub edginess: EdginessLevel,
    /// Backstory snippet.
    pub backstory: String,
    /// Active model name (from `LlmConfig::model`).
    pub active_model: String,
    /// Provider kind string (e.g. "Ollama" or "None").
